...
unsigned int sf_process_check_sum_call( int nlhs, mxArray * plhs[], int nrhs,
  const mxArray * prhs[] )
{
  extern unsigned int sf_test_bodyjacob_process_check_sum_call( int nlhs,
    mxArray * plhs[], int nrhs, const mxArray * prhs[] );
  if (sf_test_bodyjacob_process_check_sum_call(nlhs,plhs,nrhs,prhs))
    return 1;
  return 0;
}
...
{
  extern unsigned int sf_test_bodyjacob_autoinheritance_info( int nlhs, mxArray *
  ...
  if (strcmp(machineName, "test_bodyjacob") == 0) {
    const mxArray *newRhs[2] = { NULL, NULL };

    newRhs[0] = prhs[0];
    newRhs[1] = prhs[2];
    return sf_test_bodyjacob_autoinheritance_info(nlhs,plhs,2,newRhs);
  }
...
unsigned int sf_process_get_eml_resolved_functions_info_call( int nlhs, mxArray *
  plhs[], int nrhs, const mxArray * prhs[] )
{
  extern unsigned int sf_test_bodyjacob_get_eml_resolved_functions_info( int
  ...
  if (strcmp(machineName, "test_bodyjacob") == 0) {
    ...
    newRhs[0] = prhs[0];
    newRhs[1] = prhs[2];
    return sf_test_bodyjacob_get_eml_resolved_functions_info(nlhs,plhs,2,newRhs);
  }
...
extern unsigned int sf_test_bodyjacob_method_dispatcher(SimStruct *S, unsigned
...
unsigned int sf_machine_global_method_dispatcher(SimStruct *simstructPtr, const
  ...
  method, void *data)
{
  if (!strcmp(machineName,"test_bodyjacob")) {
    return(sf_test_bodyjacob_method_dispatcher(simstructPtr,chartFileNumber,
...
extern void test_bodyjacob_terminator(void);
...
extern void test_bodyjacob_initializer(void);
extern void test_bodyjacob_register_exported_symbols(SimStruct* S);
extern void test_bodyjacob_debug_initialize(void);
void sf_register_machine_exported_symbols(SimStruct* S)
{
  test_bodyjacob_register_exported_symbols(S);
}
...
void sf_machine_global_initializer(SimStruct* S)
{
  ...
  if (sfMachineGlobalInitializerCallable) {
    ...
    if (!simModeIsRTWGen) {
      test_bodyjacob_debug_initialize();
    }

    test_bodyjacob_initializer();
  }
